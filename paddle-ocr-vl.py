import os
import time
import math
import torch
from PIL import Image
from transformers import AutoModelForCausalLM, AutoProcessor
from multiprocessing import Process, set_start_method

# ---- 전역 설정 ----
MODEL_NAME = "PaddlePaddle/PaddleOCR-VL"
INPUT_FOLDER = 'data/New_sample/원천데이터/인.허가/5350109/1994/'
OUTPUT_BASE_PATH = '/workspace/output/paddleocr-vl/1994/'
GPU_IDS = [0, 1]  # 사용할 GPU 번호 리스트
TASK = "ocr"  # 'ocr' | 'table' | 'chart' | 'formula'

PROMPTS = {
    "ocr": "OCR:",
    "table": "Table Recognition:",
    "formula": "Formula Recognition:",
    "chart": "Chart Recognition:",
}

def worker_process(gpu_id, file_list):
    """
    각 GPU에서 실행될 개별 프로세스 함수
    """
    # 해당 프로세스가 사용할 GPU 지정
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    DEVICE = "cuda" # 이미 CUDA_VISIBLE_DEVICES로 제한했으므로 "cuda" 사용

    print(f"[GPU {gpu_id}] 모델 로딩 시작 (할당량: {len(file_list)}장)")

    try:
        # 1. 모델 및 프로세서 로드
        model = AutoModelForCausalLM.from_pretrained(
            MODEL_NAME, 
            trust_remote_code=True, 
            torch_dtype=torch.bfloat16
        ).to(DEVICE).eval()

        processor = AutoProcessor.from_pretrained(
            MODEL_NAME,
            trust_remote_code=True,
            min_pixels=256*28*28, # 필요에 따라 조절
            max_pixels=1024*28*28 # OOM 방지를 위해 조절
        )

        for filename in file_list:
            image_path = os.path.join(INPUT_FOLDER, filename)
            save_name = os.path.splitext(filename)[0]
            output_dir = os.path.join(OUTPUT_BASE_PATH, save_name)
            os.makedirs(output_dir, exist_ok=True)

            try:
                torch.cuda.reset_peak_memory_stats()
                start_time = time.time()

                # 2. 이미지 로드 및 전처리
                image = Image.open(image_path).convert("RGB")
                
                messages = [
                    {
                        "role": "user",
                        "content": [
                            {"type": "image", "image": image},
                            {"type": "text", "text": PROMPTS[TASK]},
                        ]
                    }
                ]

                inputs = processor.apply_chat_template(
                    messages,
                    tokenize=True,
                    add_generation_prompt=True,
                    return_dict=True,
                    return_tensors="pt"
                ).to(DEVICE)

                # 3. 추론 (Inference)
                with torch.no_grad():
                    outputs = model.generate(**inputs, max_new_tokens=1024)
                
                decoded_output = processor.batch_decode(outputs, skip_special_tokens=True)[0]

                # 4. 결과 저장 (텍스트 결과 저장)
                with open(os.path.join(output_dir, "ocr_result.txt"), "w", encoding="utf-8") as f:
                    f.write(decoded_output)

                duration = time.time() - start_time
                max_vram = torch.cuda.max_memory_allocated() / (1024 ** 3)

                # 개별 성능 리포트 저장
                with open(os.path.join(output_dir, "performance.txt"), "w") as f:
                    f.write(f"GPU ID: {gpu_id}\n")
                    f.write(f"File: {filename}\n")
                    f.write(f"Time: {duration:.2f}s\n")
                    f.write(f"Peak VRAM: {max_vram:.2f}GB\n")

                print(f"[GPU {gpu_id}] 성공: {filename} ({duration:.1f}s)")

            except Exception as e:
                print(f"[GPU {gpu_id} 오류 - {filename}] {e}")
                continue

            # 캐시 정리
            torch.cuda.empty_cache()

    except Exception as e:
        print(f"[GPU {gpu_id} 치명적 오류] 모델 로드 실패: {e}")

def main():
    # 처리할 파일 목록 확보
    valid_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff')
    if not os.path.exists(INPUT_FOLDER):
        print(f"경로가 존재하지 않습니다: {INPUT_FOLDER}")
        return

    all_files = sorted([f for f in os.listdir(INPUT_FOLDER) if f.lower().endswith(valid_extensions)])

    if not all_files:
        print("처리할 이미지가 없습니다.")
        return

    print(f"총 {len(all_files)}개의 이미지를 {len(GPU_IDS)}대의 GPU로 분할 처리합니다.")

    # 파일을 GPU 개수만큼 균등하게 분할
    num_gpus = len(GPU_IDS)
    chunk_size = math.ceil(len(all_files) / num_gpus)
    chunks = [all_files[i:i + chunk_size] for i in range(0, len(all_files), chunk_size)]

    processes = []

    # 각 GPU별로 프로세스 생성 및 시작
    for i, gpu_id in enumerate(GPU_IDS):
        if i < len(chunks):
            p = Process(target=worker_process, args=(gpu_id, chunks[i]))
            p.start()
            processes.append(p)
            time.sleep(5) # 모델 로딩 시 VRAM 피크 충돌 방지를 위한 간격

    # 모든 프로세스 종료 대기
    for p in processes:
        p.join()

    print("\n[모든 작업이 완료되었습니다.]")

if __name__ == '__main__':
    # CUDA 멀티프로세싱 안정성을 위해 'spawn' 방식 사용
    try:
        set_start_method('spawn', force=True)
    except RuntimeError:
        pass
    main()